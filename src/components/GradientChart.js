...
const GradientChart = ({ height, width, id }) => {
  ...
  const Glyph = ({ left, top, type, profileURL, staticBar }) => (
    <CustomGlyph left={left} top={top}>
      <foreignObject
        ...
        onMouseMove={() => {
          showTooltip({
            tooltipData: staticBar,
            tooltipLeft: xXScale(getXValue(staticBar.data)),

            tooltipTop: yYScale(getYValue(staticBar.data)),
          });
        }}
      >
        <img
